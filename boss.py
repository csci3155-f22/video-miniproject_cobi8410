...
class Boss():
	...
	def render(self, surface):
		...
		if self.state == 0 or self.state == -1: #if idle
			...
			self.getFrame(self.win_sheet, 10, 24)
		elif self.state == -3:
...
class grizzlyBear(Boss):
	def __init__(self, game_size):
		...
		self.win_sheet = pygame.transform.scale(pygame.image.load("assets/boss/grizzly_bear/grizzly_bear_win_sheet.png"), (game_size[0]*24, game_size[1])).convert_alpha() #sheet for win animation
		self.lose_sheet = pygame.transform.scale(pygame.image.load("assets/boss/grizzly_bear/grizzly_bear_lose_sheet.png"), (game_size[0], game_size[1])).convert_alpha()
		self.current_attack = None
